...
def _wrap_traced(node_name: str, fn: Callable[[dict], dict[str, Any]]) -> Callable[[dict], dict[str, Any]]:
    """Оборачивает узел: фиксирует текущий узел и путь прохождения в scratch."""
    def _inner(state: dict[str, Any]) -> dict[str, Any]:
        scratch_in = dict(state.get("scratch", {}) or {})
        path = list(scratch_in.get("path", []))
        ...
        out = fn(state2) or {}
        scratch_out = dict(out.get("scratch", {}) or {})
        scratch_out.setdefault("path", path)
        scratch_out.setdefault("current_node", node_name)
        out["scratch"] = scratch_out
        return out
    return _inner
...
def build_graph() -> StateGraph[AgentState]:
    ...
    g.add_conditional_edges(
        ...
        {"intro": "intro", "classify_lpr": "classify_lpr"},
    )

    g.add_edge("intro", "classify_lpr")

    g.add_conditional_edges(
        "classify_lpr",
        lambda s: s.get("route", "unknown"),
        {"is_lpr": "lpr_prompt", "not_lpr": "not_lpr", "unknown": "agent"},
    )

    g.add_edge("lpr_prompt", "classify_reason")
    ...
    g.add_edge("propose_pay_within_week", "classify_agreement")
    g.add_conditional_edges(
        "classify_agreement",
        lambda s: s.get("route", "unknown"),
    ...
    g.add_edge("restructuring", "finalize")
    ...
    g.add_edge("send_closings", "classify_reason")
    g.add_edge("need_recon", "classify_reason")
    g.add_edge("need_invoice", "create_duplicate_request")
    g.add_edge("create_duplicate_request", "classify_reason")
